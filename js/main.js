...
circles.forEach(el => {
  function progressAnimation(value, progress) {
    let percentageProgress = Math.floor(value);
    let radius = progress.getAttribute('r');
    let progressLength = 2 * Math.PI * radius;
    progress.setAttribute('stroke-dasharray', progressLength);
    progress.setAttribute('stroke-dashoffset', progressLength - progressLength * percentageProgress / 100);
    return progress;
  }
  if (el.dataset.percentage) {
    let progress = el.querySelector('.progress');
    let value = el.getAttribute('data-value') / el.getAttribute('data-full') * 100;
    progressAnimation(value, progress);
  }
  if (el.dataset.percent) {
    let progress = el.querySelector('.progress');
    let value = el.getAttribute('data-percent');
    progressAnimation(value, progress);
  }
});
...
document.addEventListener("DOMContentLoaded", event => {
  ...
  const testimonialsSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.testimonials__slider', {
    loop: true,
    slidesPerView: 1,
    spaceBetween: 30,
    navigation: {
